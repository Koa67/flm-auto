...
export async function generateStaticParams() {
  return SEGMENT_CATEGORIES.map((c) => ({ segment: c.slug }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { segment } = await params;
  const cat = SEGMENT_CATEGORIES.find((c) => c.slug === segment);
  if (!cat) return {};
  return {
    title: `${cat.label} — Explorer par segment | FLM AUTO`,
    description: `Tous les véhicules ${cat.label.toLowerCase()} : ${cat.description}. Photos, fiches techniques et comparatifs.`,
    alternates: { canonical: `/explorer/segment/${segment}` },
    openGraph: {
      images: [
        {
          url: `/api/og?title=${encodeURIComponent(cat.label)}&subtitle=Explorer%20par%20segment`,
        },
      ],
    },
  };
...
export default async function SegmentBrowsePage({ params }: Props) {
  const { segment } = await params;
  const cat = SEGMENT_CATEGORIES.find((c) => c.slug === segment);
  const dbSegment = SEGMENT_SLUG_MAP[segment];
  if (!cat || !dbSegment) notFound();
